using ArsuLeo.CS.Utils.Service.Arguments;
using System;
...
namespace ArsuLeo.CS.Utils.Model.Collections.Dictionary
{
         ...
         where TKey : notnull
    {
        ...
        public static MultiValueDictionary<TKey, TValue> FromData(
            IEnumerable<KeyValuePair<TKey, TValue>> src, IEqualityComparer<TValue>? valueComparer = null)
        {
            MultiValueDictionary<TKey, TValue> r = valueComparer is null
                ? new MultiValueDictionary<TKey, TValue>()
                : new MultiValueDictionary<TKey, TValue>(valueComparer);
        ...
        public static MultiValueDictionary<TKey, TValue> FromData(
            IEnumerable<KeyValuePair<TKey, IEnumerable<TValue>>> src, IEqualityComparer<TValue>? valueComparer = null)
        {
            MultiValueDictionary<TKey, TValue> r = valueComparer is null
                ? new MultiValueDictionary<TKey, TValue>()
                : new MultiValueDictionary<TKey, TValue>(valueComparer);
        ...
        {
            ArgumentVerifier.CantBeNull(key, nameof(key));
        ...
        public void Merge(MultiValueDictionary<TKey, TValue> toMergeWith, MergeMode mergeMode = MergeMode.MergeAll)
        {
            if (toMergeWith is null)
            {
                return;
            }
            ...
            switch (mergeMode)
            {
                case MultiValueDictionary<TKey, TValue>.MergeMode.KeepOnlyOldValues:
                    ...
                    break;
                case MultiValueDictionary<TKey, TValue>.MergeMode.KeepOnlyNewValues:
                    foreach (KeyValuePair<TKey, HashSet<TValue>> pair in toMergeWith)
                    {
                        Remove(pair.Key);
                        foreach (TValue value in pair.Value)
                    ...
                    break;
                case MultiValueDictionary<TKey, TValue>.MergeMode.MergeAll:
                default:
                    foreach (KeyValuePair<TKey, HashSet<TValue>> pair in toMergeWith)
                    {
                        foreach (TValue value in pair.Value)
        ...
        {
            return !TryGetValue(key, out HashSet<TValue>? toReturn)
